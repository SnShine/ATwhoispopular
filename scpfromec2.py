...
class MyStreamListener(tweepy.StreamListener):
    def on_status(self, status):
        ...
        logging.info("Tweet from: %s; Id: %s" % (tweet_id, tweet_from))
        print("Tweet from: %s; Id: %s" % (tweet_id, tweet_from))
        ...
        if tweet_from.lower() != MYUSERNAME and tweet_from.lower() not in BLACKLIST and not hasattr(status, "retweeted_status"):
            try:
                ...
                logging.info("Got google trends data")
...
if __name__== "__main__":
    ...
    try:
        ...
        logging.info("Stream exception: %s" % e)
        raise e
